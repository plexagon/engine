...
class HtmlRenderer implements Renderer {
  ...
  ui.Gradient createConicalGradient(ui.Offset focal, double focalRadius,
          ...
          ui.TileMode tileMode = ui.TileMode.clamp,
          Float32List? matrix]) =>
      GradientConical(focal, focalRadius, center, radius, colors, colorStops,
          tileMode, matrix);
  ...
  ui.ImageFilter createDilateImageFilter(
    ...
    throw UnimplementedError(
        'ImageFilter.dilate not implemented for HTML renderer.');
  }
  ...
  ui.ImageFilter createErodeImageFilter(
    ...
    throw UnimplementedError(
        'ImageFilter.erode not implemented for HTML renderer.');
  }
  ...
  void decodeImageFromPixels(Uint8List pixels, int width, int height,
    ...
    ui
        .createBmp(pixels, width, height, rowBytes ?? width, format)
  ...
  Future<ui.Image> createImageFromImageBitmap(
    ...
    errorListener = createDomEventListener((DomEvent event) {
      completer.completeError(
          Exception('Failed to create image from image bitmap.'));
      imageElement.removeEventListener('load', loadListener);
      imageElement.removeEventListener('error', errorListener);
    });
